#include <algorithm>		// if you have a problem with this C++  code, 
#include <cmath>			// consult your programing text book first...
#include <cstdio>
#include <cstring>
using namespace std;
		/* Forming Quiz Teams, the solution for UVa 10911 above */
			// using global variables is a bad software engineering practice,
int N, target;			//but it is OK for competitive programing 
...
if (memo[bitmask] > -0.5)		// this state has been computed befor
...
double ans = 2000000000.0;			//initialize with large value
int p1, p2;
for (p1 = 0; p1 < 2 * N; p1++)
	if (!(bitmask & (1 << p1)))
